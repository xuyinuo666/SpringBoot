...
spring:
  web:
    resources:
      static-locations: classpath:/HTMLtest/
      //默认就从HTMLtest文件夹中找资源
```
...
@PostMapping("domain")
    public String t3() {
        return "redirect:/domain.html";//重定向到视图控制器
    }

// 		registry.addViewController("/domain.html").setViewName("dashboard"); dashboard 已经被模板引擎渲染过了
```
...
public class MyMvcConfig extends WebMvcConfigurerAdapter {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        ...
        registry.addInterceptor(new MyLoginInterceptor()).
                addPathPatterns("/**").excludePathPatterns("/","/to");
    }
    @Override
    public void addViewControllers(ViewControllerRegistry registry) {
...
public class LoginController {
    ...
    public String login(@RequestParam("username") String username,
        ...
        if (!StringUtils.isEmpty(username) && "11111".equals(password)) {
...
//注册servlet
    ...
    public ServletRegistrationBean servletRegistrationBean(){
        ServletRegistrationBean bean = new ServletRegistrationBean(new ServletBean(), "/servletbean");
        return bean;
    }
    //注册Listener
    @Bean
    public ServletListenerRegistrationBean servletListenerRegistrationBean(){
        ServletListenerRegistrationBean bean2 = new ServletListenerRegistrationBean(new ListenerBean());
        return bean2;
    }
    //注册Filter
    @Bean
    public FilterRegistrationBean filterRegistrationBean(){
        FilterRegistrationBean bean3 = new FilterRegistrationBean(new FilterBean());
        bean3.setUrlPatterns(Arrays.asList("/servletbean"));
        return bean3;
    }
```
...
public class Myrealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        info.addStringPermission(user.getPerms().getPerms() + ":/topermission");
        return info;
    }
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        String s1= (String) redisUtil.get(username);
        User user = JSONArray.parseObject(s1,User.class);
        if (user == null) {
            user = userService.SelectByphone(Long.parseLong(username));
            if (user==null){
                return null;
            }
            String S = JSON.toJSONString(user);
            redisUtil.set(user.getPhoneNumber().toString(), S);
        }
        Subject subject = SecurityUtils.getSubject();
...
/shiro相关
    ...
    public ShiroFilterFactoryBean shiroFilterFactoryBean(@Qualifier("manager") DefaultWebSecurityManager manager) {
        ...
        b.setSecurityManager(manager);
        Map map = new HashMap<>();
        //添加过滤器
        //只有认证过的才能进入,首页跳login.顺序有关
        map.put("/", "anon");
        map.put("/index", "anon");
        map.put("/tologin", "anon");
        map.put("/todologin", "anon");
        map.put("/toregister", "anon");
        map.put("/todoregister", "anon");
        map.put("/logout", "logout");
        map.put("/topermission", "perms[admin:/topermission]");
        map.put("/**", "authc");
        b.setFilterChainDefinitionMap(map);
        b.setLoginUrl("/tologin");
        b.setUnauthorizedUrl("/noauthor");
        return b;
    }
```
...
<img src="C:\Users\Xugao\AppData\Roaming\Typora\typora-user-images\image-20210108115150100.png" alt="image-20210108115150100"  />
...
整合shiro时自定义redis管理器的类注意泛型,否则将不知道放入缓存的键值对

```java
public class redisCach<k,v> implements Cache<k,v>{
